# copied from Sofía Minãno-Gonzalés, https://github.com/sfmig/DeepLabCut/blob/master/auxiliary%20scripts/main_dataaugm.py
# edited to use model prefix to organize different augmentations by Jonas Håkansson 2022-07-15
...
#config_path = '/home/jonas2/DLC_files/projects/testingDLC-jonas2-2022-07-11/config.yaml'
config_path = 'D:\\Dropbox\\Geneve2022\\dlc_projects\\testingDLC-jonas2-2022-07-11\\config.yaml'
...
#create_training_dataset(config_path,Shuffles=[SHUFFLE_ID])
...
list_data_augm_type_to_pop.remove('')

cfg = read_config(config_path)
project_path = cfg["project_path"]
dlc_models_path = os.path.join(project_path, "dlc-models")
...
for augmentation in list_data_augm_type_to_pop:
    aug_path = os.path.join(project_path, 'aug_' + str(augmentation))
    aug_dlc_models = os.path.join(aug_path, "dlc-models", )
    aug_training_datasets = os.path.join(aug_path, "training-datasets")
    try:
        os.mkdir(aug_path)
    ...
    train_pose_config_file,\
        _, _ = deeplabcut.return_train_network_path(config_path,
                                                    ...
                                                    modelprefix='aug_' + str(augmentation)) # default
    
    os.makedirs(str(os.path.dirname(train_pose_config_file)))
    shutil.copyfile(base_train_pose_config_file, train_pose_config_file)

    ## edit config for each case
    list_data_augm_type_one_iter = [j for j in list_data_augm_type_to_pop if j!=augmentation]
    edits_dict = dict()
    for el in list_data_augm_type_one_iter:    
        edits_dict.update(params[el])

    edit_config(str(train_pose_config_file), edits_dict)
    edit_config(str(train_pose_config_file), {'project_path': project_path})
    ...
    deeplabcut.train_network(config_path,
                                ...
                                modelprefix='aug_' + str(augmentation))
